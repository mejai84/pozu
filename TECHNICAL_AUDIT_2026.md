...
### 23. Auditoría de Seguridad Pre-Publicación y Hardening (18 Abril 2026) ✅
- [x] **Arquitectura de Autenticación en Servidor (`src/lib/auth/server.ts`)**: Creado nuevo helper `createServerSupabase`, `requireAuth` y el wrapper `withAuth` empleando `@supabase/ssr` con persistencia de cookies. Mitigada vulnerabilidad crítica donde APIs de admin (como `/api/admin/employees`) carecían de verificación de acceso o rol real en servidor.
- [x] **Implementación de Middleware Next.js (`src/middleware.ts`)**: Creado middleware unificado para interceptar todo el tráfico hacia `/admin/*` y `/api/admin/*`, bloqueando inmediatamente accesos de red de usuarios anónimos resolviendo la protección reactiva puramente del frontend.
- [x] **Validación de Inputs Universal con Zod (`src/lib/validation/schemas.ts`)**: Migración global hacia Type-Safety extremo con `zod@4.3.6`. Creados esquemas estructurales con saneamiento estricto para: API Empleados (limites de strings y enums de roles), Checkout (montos estrictamente positivos y cap máximo), Uploads (enumeración de MIME types), y Reservas.
- [x] **Aislamiento de la DB desde Clientes (`/api/reservar/route.ts`)**: Extirpado del cliente (`src/app/reservar`) el envío directo de consultas PQR a Supabase. Centralizado tras nuevo endpoint API servidor, el cual se auto-beneficia del limitador de tráfico y RLS verificado por IP.
- [x] **Engine de Rate-Limiting Dinámico en Memoria (`src/lib/rate-limit/limiter.ts`)**: Infraestructura zero-deps para single-instance deploys. Protege contra DDOS básicos e intentos exhaustivos: Checkout `paymentLimiter` (5 req/m), Reservas `formLimiter` (10 req/m), Uploads `uploadLimiter` (20 req/m), y APIs Admin `adminLimiter` (30 req/m). 
- [x] **Sanitización de Errores Vía Network**: Editadas las APIs críticas (Checkout, Empleados, Uploads, Reservas, Webhooks) deteniendo la exposición e inyección de sentencias o fallos internos directos en los returns de Next.js (`error.message`). En su sustitución reinyectados throw messages opacos con error tracking unívoco en el Server Shell.
- [x] **Cierre de Vulnerabilidades SSRF via Axios**: Refactorizadas las dependencias en `package.json` (`npm update axios`) resolviendo 2 flags moderados de seguridad por normalización Bypass para meta-queries internas a n8n y Supabase.
