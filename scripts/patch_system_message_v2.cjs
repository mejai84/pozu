...
const outputPath = 'c:\\Users\\Cesar\\Downloads\\Pozu_FINAL_LISTO_PRODUCCION_V2.json';
...
try {
  ...
  if (agentNode && agentNode.parameters && agentNode.parameters.options) {
    ...
    // Construimos el nuevo prompt usando la base del usuario MÁS las fórmulas dinámicas.
    // Omitimos la regla contradictoria ('No puedo ver el progreso') porque el usuario YA añadió la herramienta!
...
- NUNCA confirmes disponibilidad sin consultar_disponibilidad.\` }`;
    ...
    console.log('✅ System message reconstruido.');
  }

  // Ensure tool connection exists
  if (!data.connections['consultar_estado_pedido']) {
    ...
    console.log('✅ Fix: Herramienta consultar_estado_pedido conectada al Agente.');
  }

  fs.writeFileSync(outputPath, JSON.stringify(data, null, 2));
  console.log('✅ JSON V2 Guardado en: ' + outputPath);
