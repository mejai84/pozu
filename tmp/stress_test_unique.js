...
async function sendRequest(id) {
    const data = {
        ...
        session_id: `unique_session_${id}`, // EVERY REQUEST HAS A UNIQUE SESSION
        user_name: `StressBot_${id}`,
        text: `Hola! Dime qué hamburguesas tienes disponibles? #${id}`
    };
    ...
    try {
        ...
        const end = Date.now();
        console.log(`[Request ${id}] Success! (${end - start}ms) - Response: ${JSON.stringify(response.data).substring(0, 50)}...`);
    } catch (error) {
        const end = Date.now();
...
async function runStressTest() {
    console.log('--- STARTING UNIQUE SESSIONS STRESS TEST ---');
    const requests = [];
    for (let i = 1; i <= 5; i++) {
